...
PAPERS=[
 dict(slug='before-i-knew-what-to-measure',title_en='Before I Knew What to Measure: Following an Intuition Through Behavioral Data, Space Weather, and the Search for What Actually Changed',title_fa='پیش از آن‌که بدانم چه چیزی را باید اندازه بگیرم',source_en='2026/before-i-knew-what-to-measure/releases/v0.1/paper.en.md',source_fa='2026/before-i-knew-what-to-measure/releases/v0.1/paper.fa.md',repo='2026/before-i-knew-what-to-measure',cover='measure',kicker_en='Evidence-Backed Working Paper',kicker_fa='مقالهٔ کاری مبتنی بر شواهد',date_en='31 August 2026',date_fa='۳۱ اوت ۲۰۲۶',date_iso='2026-08-31',status_en='Frozen research snapshot',status_fa='تصویرِ فریز‌شدهٔ پژوهش',version='v0.1',topics=['Anomaly Detection','Causal Inference','Behavioral Traces','Falsification'],artifacts=[('Open Research Record','2026/before-i-knew-what-to-measure'),('Release notes · v0.1','2026/before-i-knew-what-to-measure/releases/v0.1/RELEASE_NOTES.md'),('Source audit · v0.1','2026/before-i-knew-what-to-measure/references/audit-sources-v0.1.md'),('Figure 1 · behavioral timeline','2026/before-i-knew-what-to-measure/figures/figure-01-behavior-space-weather.svg')]),
...
def artifact_links(p,fa):
    ...
    def artifact_url(path):
        ...
        return f'{GITHUB}/{"tree" if not Path(path).suffix else "blob"}/main/{path}'
    rows=''.join(f'<a href="{artifact_url(path)}" target="_blank" rel="noopener"><span>{html.escape(label)}</span><b>↗</b></a>' for label,path in arts)
...
def write_sitemap():
    ...
    lines=['<?xml version="1.0" encoding="UTF-8"?>','<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">']+[f'  <url><loc>{html.escape(u)}</loc><lastmod>2026-08-31</lastmod></url>' for u in urls]+['</urlset>']; (OUT/'sitemap.xml').write_text('\n'.join(lines),encoding='utf-8')
