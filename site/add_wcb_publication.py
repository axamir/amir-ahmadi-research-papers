...
META={
'en':dict(title='We Are Code That Breathes',subtitle='A Provenance-Preserving Case Study of Claim Evolution in Persistent Human–Generative-AI Collaboration',kicker='Protocol Construction · Case Demonstration',status='Release Candidate v0.3',date='August 2026',topics=['Research Provenance','Human–AI Collaboration','Claim Evolution','PRCEP']),
'fa':dict(title='ما کدی هستیم که نفس می‌کشد',subtitle='مطالعه‌ی موردیِ حفظ منشأ درباره‌ی تکامل ادعا در همکاری پایدار انسان و هوش مصنوعی مولد',kicker='ساخت پروتکل · نمایش موردی',status='نسخه کاندید انتشار v0.3',date='اوت ۲۰۲۶',topics=['منشأ پژوهش','همکاری انسان و هوش مصنوعی','تکامل ادعا','PRCEP'])}
...
def page(lang):
    fa=lang=='fa'; m=META[lang]; source=FA if fa else EN; article,plain=render(source)
    url=f"{BASE}/{'fa/' if fa else ''}papers/{SLUG}/"; alt=f"{BASE}/{'papers' if fa else 'fa/papers'}/{SLUG}/"
    home=f"{BASE}/{'fa/' if fa else ''}"; source_url=f"{GITHUB}/blob/main/{REPO_PATH}/{source.name}"; repo=f'{GITHUB}/tree/main/{REPO_PATH}'
    desc=html.escape((plain[:260]+'…') if len(plain)>260 else plain,quote=True)
    ...
    boundary=('این نسخه فارسی از نظر قدرت ادعا، عدم‌قطعیت و مرزهای علمی با نسخه انگلیسی هم‌تراز است.' if fa else 'Protocol-construction and case-demonstration study. PRCEP is a protocol candidate, not a validated protocol.')
    return f'''<!doctype html><html lang="{lang}"{direction}><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(m['title'])} — Amir Ahmadi Research</title><meta name="description" content="{desc}"><link rel="canonical" href="{url if not fa else BASE+'/papers/'+SLUG+'/'}"><link rel="alternate" hreflang="en" href="{BASE}/papers/{SLUG}/"><link rel="alternate" hreflang="fa" href="{BASE}/fa/papers/{SLUG}/"><link rel="stylesheet" href="{BASE}/assets/paper.css"><link rel="stylesheet" href="{BASE}/assets/polish.css"><meta property="og:type" content="article"><meta property="og:title" content="{html.escape(m['title'],quote=True)}"><meta property="og:description" content="{desc}"><meta property="og:url" content="{url}"></head><body class="reading-page {'rtl' if fa else ''}"><header class="top"><div class="shell nav"><a class="brand" href="{home}">@@ Amir Ahmadi Research</a><nav class="navlinks"><a href="{home}">{allp}</a><a href="{repo}" target="_blank" rel="noopener">GitHub ↗</a></nav></div></header><main><section class="reading-hero"><div class="shell reading-shell"><div class="eyebrow">{html.escape(m['kicker'])}</div><h1>{html.escape(m['title'])}</h1><p class="lede">{html.escape(m['subtitle'])}</p><p class="reading-note">{html.escape(boundary)}</p><div class="reading-meta"><span>{m['date']}</span><span>ARP-WCB-2026-01</span><span>{html.escape(m['status'])}</span></div><div class="topic-row">{pills}</div><div class="actions"><a class="btn primary" href="#paper">{read} ↓</a><a class="btn" href="{alt}">{'English edition' if fa else 'نسخه فارسی'} ↗</a><a class="btn" href="{source_url}" target="_blank" rel="noopener">{src} ↗</a></div><div class="reading-cover relation"><span>PRCEP · PROVENANCE · CLAIM EVOLUTION</span><strong>{html.escape(m['title'])}</strong><small>ARP-WCB-2026-01 · RC v0.3</small></div></div></section><section class="shell reading-shell reading-layout" id="paper"><article class="markdown-body">{article}</article><aside class="reading-side"><div class="side-card publication-card"><div class="side-label">{'هویت انتشار' if fa else 'Publication identity'}</div><dl><dt>{'نویسنده' if fa else 'Author'}</dt><dd>Amir Ahmadi</dd><dt>{'سند' if fa else 'Document'}</dt><dd>ARP-WCB-2026-01</dd><dt>{'وضعیت' if fa else 'Status'}</dt><dd>{html.escape(m['status'])}</dd></dl></div><div class="side-card provenance-card"><div class="side-label">{'رکورد پژوهش' if fa else 'Research record'}</div><a href="{repo}" target="_blank" rel="noopener">{folder} ↗</a><a href="{source_url}" target="_blank" rel="noopener">{src} ↗</a><a href="{GITHUB}/blob/main/{REPO_PATH}/REFERENCES.md" target="_blank" rel="noopener">References ↗</a><a href="{GITHUB}/blob/main/{REPO_PATH}/AI_DISCLOSURE.md" target="_blank" rel="noopener">AI disclosure ↗</a></div></aside></section></main><footer><div class="shell">© 2026 Amir Ahmadi · Independent Research · <a href="{GITHUB}">Source archive</a></div></footer><script src="{BASE}/assets/app.js"></script></body></html>'''
...
def inject_card(path,lang):
    ...
    # Safe fallback: place a publication callout before the footer rather than failing the build.
...
def main():
    ...
    print('Added ARP-WCB-2026-01 publication pages and landing entry.')
